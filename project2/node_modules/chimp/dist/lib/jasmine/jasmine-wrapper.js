...
var _screenshotHelper = require('../screenshot-helper');

var _screenshotHelper2 = _interopRequireDefault(_screenshotHelper);

var _booleanHelper = require('../boolean-helper');

var _booleanHelper2 = _interopRequireDefault(_booleanHelper);
...
new _fibers2.default(function runJasmineInFiber() {
  ...
  jasmine.jasmine.getEnv().addReporter({
    specDone: function specDone(result) {
      if (_screenshotHelper2.default.shouldTakeScreenshot(result.status)) {
        if (_booleanHelper2.default.isTruthy(process.env['chimp.saveScreenshotsToDisk'])) {
          var affix = result.status !== 'passed' ? ' (failed)' : '';
          var fileName = result.fullName + affix;
          _screenshotHelper2.default.saveScreenshotsToDisk(fileName, projectDir);
        }
      }
    }
  });
